...
| Host playlists (M3U in index DB; local reorder/shuffle → device on next Sync) | `domain/playlist_m3u.py` (`move_paths`), `domain/playlist_shuffle.py` (artist merge, album-block merge, Spotify dither), `infra/playlists.py` (`move_paths_in_playlist`, `replace_playlist_tracks`), Playlists tab ↑/↓ + Shuffle context menu in `ui/window.py` / `ui/controllers.py`, dialog `ui/dialogs.py` (`ask_add_to_playlist`) |
