...
| Host playlists (M3U in index DB; local reorder → device on next Sync) | `domain/playlist_m3u.py` (`move_paths`), `infra/playlists.py` (`move_paths_in_playlist`), Playlists tab ↑/↓ + context menus in `ui/window.py` / `ui/controllers.py`, dialog `ui/dialogs.py` (`ask_add_to_playlist`) |
