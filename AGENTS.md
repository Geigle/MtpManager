...
| Send Video (Video 120 / TV 124) | `domain/device_profile.VideoEncodeProfile` + ZEN profile in `device_profiles.py`; encode `infra/ffmpeg_video.py`; `app/device_ops.prepare_and_send_video`; UI `dialogs.ask_video_destination` + controllers; `tests/test_send_video.py` |
