...
| Library index (SQLite + GUID; multi-root) | `infra/library_index.py`, `domain/library.py` (`root_paths`), `app/scan_library.py`, `domain/track_id.py`, `infra/app_paths.py`. Prefer `upsert_library_tracks` for one-off GUID fixes (Send/Stage Video). **Never** call full `save_library_index` after a video send — it rewrites every row and (via private `.tuneout` hooks) every library folder → multi-minute beach ball on large/network libraries. |
...
| App config (send format, encode presets, …) | `infra/app_config.py` (`config.json`); encode model `domain/audio_encode.py`; video podcast prefs `domain/video_encode.PodcastVideoEncodeSettings`; ffmpeg map `infra/ffmpeg_transcode.py`; Config → Config…; podcast audio/video encode defaults in Config → Podcast Settings; audiobook encode in Config → Audiobook Encode…; per-show audio+video encode via Podcasts tab show context **Encode Settings…** |
| Quality ceiling / no up-convert | `domain/audio_quality.py` (`decide_action`, tier map); wired in `app/transfer.prepare_track`; tests `tests/test_audio_quality.py` |
...
| Send Video (Video 120 / TV 124) | Recipe tabs + resolutions in `device_profile.py` / `device_profiles.py`; catalog + `apply_resolution` / `apply_audio_settings` / `apply_video_quality` in `domain/video_encode.py`; encode `infra/ffmpeg_video.py` (mpeg4 qscale + optional slow HQ flags); `app/device_ops.prepare_and_send_video` / `stage_video_for_sync`; UI `dialogs.ask_video_destination` (Send or Stage); podcast video prefs reuse the same quality axes; staged encodes `infra/staged_videos.py` (1-week TTL; Transfer → Sync Staged Videos…; library ctx Stage/Sync Staged); `tests/test_video_encode.py`, `tests/test_send_video.py`, `tests/test_staged_videos.py` |
