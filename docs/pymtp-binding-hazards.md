...
| **Get_Tracklisting bulk is slow on ZEN** | Full `LIBMTP_Get_Tracklisting*` can run **hours** on large libraries (progress only inside C until return); historically incomplete + zero-packet noise | **Product choice (restored):** List Tracks / Delete All **use** bulk tracklisting by default for complete tags on smaller devices / soak tests. Fast `list_tracks_from_files` (filelisting + filter) remains for tooling. Expect long USB quiet cooldown after bulk list |
