...
| **Concurrent USB use** | Auto-connect poll + transfer + seed/enrich on same session without locking | Race / poison — **Fixed:** exclusive `DeviceIoGate` (`app/device_io_gate.py`); every USB path acquires; poll only `try_acquire` when `can_auto_probe` |
| **Disconnect while worker runs** | Main-thread vs bg poll | Stale session, double free class bugs — **mitigated:** manual disconnect / Stable Mode `steal()` the gate so release ownership is clear |
...
| **Post-listing auto-connect kill** | ~~was~~ list_tracks ok, then poll `session_alive`/`get_device_info` on a recovering bus → disconnect → `Could not close session!` / endpoint errors | **Fixed:** gate hold for whole job + quiet cooldown after release + soft-fail probes (2 strikes) + connect/auto-connect identity-only; full `get_info` is Device → Device Info only with per-field soft-fail |
| **Cached modelname as liveness** | ~~was~~ `session_alive` called `get_modelname` (libmtp returns **cached** deviceinfo — no USB) so after first connect physical unplug never failed the probe; auto-reconnect never ran until Device → Disconnect | **Fixed:** `session_alive` uses `LIBMTP_Get_Storage` (real PTP); disconnect force-clears the device pointer if Release errors |
