...
```text
[index load | Select/Scan Library] → scan_library → Library[Track]
     → user action → transfer_track(s)
     → (optional) FFmpegTranscoder → Transport.send_track
```

Details: [transfer-and-modes.md](./transfer-and-modes.md). Durable library index lives under the app data dir (`infra/app_paths.py` + `infra/library_index.py`).
