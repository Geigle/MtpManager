...
**Device admin (experimental, implemented):** **List Files** (live `get_filelisting`); **List Tracks** (mtp-tracks style: filelisting + per-id `get_track_metadata` for tags; bulk `get_tracklisting` incomplete on ZEN); single **Delete Track** / **Get File|Track Info** (live pickers); **Delete All Tracks** (live track list + batch `delete_object`, fatal abort). Listings run on `_run_device_bg` so USB walks do not freeze Tk.
