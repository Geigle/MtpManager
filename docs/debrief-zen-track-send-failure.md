...
- ~~PyMTP still PTP `02ff` / “Could not send object” after filetype fix.~~ **Mitigated:** (1) set real ctypes `argtypes` for `LIBMTP_Send_Track_From_File`, (2) fix `Dump_Errorstack` NULL-device PANIC, (3) on remaining 02ff, release the PyMTP session and **retry once via mtp-sendtr** (same contract as stable mode), then reconnect.  
