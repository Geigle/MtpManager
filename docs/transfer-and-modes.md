...
| Podcast encode override | **Library → Podcast Settings…** → `config.json` + per-show in library DB | Optional default `podcast_audio_encode` for genre Podcast. Optional per-show `audio_encode_json` on the subscription (talk shows low bitrate; RPG/music shows higher). Precedence: per-show → podcast default → Config `audio_encode`. |
