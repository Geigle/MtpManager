...
| Audiobook encode override | **Config → Audiobook Encode…** → `config.json` | Optional `audiobook_audio_encode` recipe. When set, **only** tracks under the Audiobooks tab (genre Audiobook) use it instead of the global Config encode. |
| Podcast encode override | **Config → Podcast Settings…** (default) + **Podcasts tab → show context → Encode Settings…** (per-show) | **Audio:** default `podcast_audio_encode` in `config.json`; per-show `audio_encode_json` + `playback_speed` (talk shows low bitrate; RPG/music higher quality; speed via ffmpeg `atempo`). Precedence: per-show → podcast default → Config `audio_encode`. **Video:** optional `podcast_video_encode` (recipe + resolution + qscale/slow quality + muxed-audio ladder) and per-show `video_encode_json`. Precedence: per-show → podcast video default → device `DeviceVideoOptions` defaults. Distinct from library Send Video last-used prefs (`video_encode_qscale_v` / `video_encode_slow`). |
| Track context menu | Right-click listbox row | Sync this track / Sync Album / Sync all from Artist; **Special Sync…** (experimental tools only) |
| Special Sync… | Context menu (Music / Audiobooks / Podcasts) when **Enable Experimental Tools** is on | Dialog: encode, metadata overrides (host tags untouched), parent folder id, create folders (PyMTP only), GUID vs human ObjectFileName (default stem when GUID off). Session-only. Stable Mode allowed without Create Folders. Pipeline: `domain/special_sync.py` + `app/transfer` overrides. |
...
| Right-click track | **Sync N selected tracks** (when multi-select), **Sync this track**, **Sync Album**, **Sync all from Artist** (global format + active mode transport). **Video rows:** **Stage for Sync…** (same encode dialog as Send Video, confirm button **Stage**; writes under `{data_dir}/staged_videos/`, 1-week TTL) — label becomes **Sync Staged** when that source is already staged. Right-click inside a multi-selection keeps the selection. |
| **Transfer** menu | **Sync Entire Library**; **Sync Folder…**; **Sync Selected Tracks** (multi-select batch); **Resume Sync**; **Sync Staged Videos…** (send all staged encodes, delete each after success); **Package Retail Demos…** / **Restore Retail Package…** (export → zip Creative demos; zip → player); **Cancel Current Job** |
| **Device** menu | **Standard** (PyMTP mode): Connect, Disconnect, Device Info (only place to edit device name — applied on close if changed), Create Folder…, **Send Video…** (standard tool: pick host video; choose **Video** 120 or **TV** 124; optional **Encode for device** from `DeviceProfile.video_options` (ZEN Vision:M only — hidden on generic): notebook of mutually exclusive **container + video codec** recipes (default **AVI · XviD · MP3**, plus **AVI · DivX · MP3**; **WMV · WMA** broken/hidden unless Config → **Show broken video encode presets**); orthogonal **resolution** picker from the device profile (ZEN: **160×120 / 320×240 / 640×480**, default **320×240**); **audio** format/quality ladder same as music/podcasts, clamped per recipe (AVI→MP3, WMV→WMA); last choices persisted in `config.json`; ZEN caps at 30 fps unless **Ignore max frame rate (experimental)**; background job with determinate progress; skip encode when source already matches; sanitized basename, no library GUID), Refresh Device Index…. **Experimental tools** (only when **Config → Enable Experimental Tools** is on): List Folders, List Files, List Tracks (fast `get_filelisting` + media filter; optional **Load tags for selection** via `get_track_metadata`), Get Tracks from Device…, Delete Track, Get File Info, Get Track Info, Delete All Tracks… (batch `delete_object`, fatal abort). Device menu is PyMTP-only (disabled under Stable Mode). |
