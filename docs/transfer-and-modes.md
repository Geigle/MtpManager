...
| **Config** menu | **Stable Mode**; **Enable Experimental Tools** (default off: hides experimental Device/Transfer items and experimental Config toggles such as artist/album folders); podcast keep/reveal/clear; **Config…** (send format, broken video presets). |
