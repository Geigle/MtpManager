...
| Format / encode presets | **Config → Config…** → `{data_dir}/config.json` | Durable `send_format` + full `audio_encode` recipe (bitrate, VBR, channels, sample rate, …). Named ladders per format (MP3/WMA/WAV/FLAC/AAC/OGG/Opus); Advanced view for granular controls. Device profiles restrict formats (ZEN Vision:M: MP3/WMA/WAV; generic unrestricted). All Sync actions use the saved recipe via `FFmpegTranscoder.convert(..., settings=…)` |
