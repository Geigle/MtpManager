...
| Podcast encode override | **Config → Podcast Settings…** (default) + **Podcasts tab → show context → Encode Settings…** (per-show) | Default `podcast_audio_encode` in `config.json`. Per-show `audio_encode_json` + `playback_speed` on the subscription (talk shows low bitrate; RPG/music higher quality; speed via ffmpeg `atempo`). Precedence: per-show → podcast default → Config `audio_encode`. |
