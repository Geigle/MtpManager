...
| Audiobook encode override | **Library → Podcast Settings…** → same `config.json` | Optional `audiobook_audio_encode` recipe. When set, **only** tracks under the Audiobooks tab (genre Audiobook) use it instead of the global Config encode. Music/podcasts/etc. still use `audio_encode`. |
