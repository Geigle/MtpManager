...
# container AVI, MPEG-4 Part 2 XVID (or DX50), yuv420p, usually 640×480,
# 25 or ~29.97 fps, single MP3 stereo 44.1 kHz. Encode path uses stock ffmpeg
# mpeg4 + vtag XVID (no libxvid). ZEN-specific: cap fps at 30 when source is
# higher; otherwise keep source (base VideoEncodeProfile has max_fps=0).
# See Device → Send Video and docs/transfer-and-modes.md.
ZEN_VISION_M_VIDEO = VideoEncodeProfile(
    ...
    width=640,
    height=480,
    max_fps=30.0,  # ZEN-only cap; default profiles leave source fps alone
    qscale_v=5,
    ...
    audio_channels=2,
    summary=(
        "AVI · XVID · 640×480 · keep source fps (ZEN cap 30) · "
        "MP3 stereo 44.1 kHz (retail demos)"
    ),
)
